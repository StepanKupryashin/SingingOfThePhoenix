init:
    ...
    transform leap(dt=.4, dyz=0.01, dxz=0.005):
        yzoom 1.0
        easein dt*0.35 yzoom 1.0+dyz xzoom 1.0-dxz
        easeout dt*0.35 yzoom 1.0 xzoom 1.0
        easein dt*0.15 yzoom 1.0-dyz xzoom 1.0+dxz
        easeout dt*0.15 yzoom 1.0 xzoom 1.0

    # относительное положение на экране по х(/y)-координате
    transform at_axy(ax=.5, ay=1.0):
        align (ax, ay)

    # положение на экране по х(/y)-координате
    transform at_pos(ax=.5, ay=1.0):
        anchor(.5, 1.0) pos(ax, ay)
...
init -2 python:
    # директория для хранение звуков
    audio_dir = config.play_channel

    music_dir = "music"

    # чтобы str() не вылетала на линуксе
    mystr = eval("lambda i: '%s' % i")

    # смена языка
    def set_lang(language="toggle"):
        if language == "toggle":
    ...
    def get_size(displayable):
        w, h = renpy.render(img2disp(displayable), 0, 0, 0, 0).get_size()
        return int(w), int(h)
    ...
    # with flash("#822")
    # цвет, длительность вспышки, пауза на середине, когда цвет залил экран
    def flash(color="#fff", t=.5, w=0):
        renpy.transition(Fade((t - w) * .5, w, (t - w) * .5, color=color))
...
# автоматическое объявление анимации
    ...
    def show_forever(screen):
        renpy.show_screen(screen, _transient=False)
    ...
    def delete_saves(confirm=True):
        if confirm:
            layout.yesno_screen(message="Удалить все сохранения?", yes=DeleteSavesNow(), no=NullAction())
        else:
            delete_saves_now()
    ...
    def sfxplay(name, channel="effect", loop=True, fadein=1.5, fadeout=1.5, ext="ogg"):
        if name:
    ...
    def rndplay(mname, fadein=1.5, fadeout=1.5, loop=True, channel="music", ext="ogg"):
